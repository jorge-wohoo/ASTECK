    ...
    def _compute_infile(self):
        for record in self:
            if record.move_type == 'out_invoice' and record.state in ('posted', 'cancel') and record.amount_tax is not None and record.create_date is not None:
                ...
                xmlstr+='<dte:Emisor AfiliacionIVA="GEN" CodigoEstablecimiento="'+ str(record.create_uid.x_studio_numero_de_sede) + '" CorreoEmisor="'+ str(record.company_id.email) + '" NITEmisor="'+ str(record.company_id.vat) +'" NombreComercial="'+ str(record.company_id.name) +'" NombreEmisor="'+ str(record.company_id.name) +'">'
                xmlstr+='<dte:DireccionEmisor>'
                xmlstr+='<dte:Direccion>' + str(record.create_uid.x_studio_direccion_de_sede) + '</dte:Direccion>'
                xmlstr+='<dte:CodigoPostal>' + str(record.create_uid.x_postal_code) + '</dte:CodigoPostal>'
                xmlstr+='<dte:Municipio>'+ str(record.create_uid.x_municipio) + '</dte:Municipio>'
                xmlstr+='<dte:Departamento>'+ str(record.create_uid.x_departamento) +'</dte:Departamento>'
